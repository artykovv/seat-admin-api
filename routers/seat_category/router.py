...
async def create_seat_category(data: SeatCategoryCreate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(is_superuser=True))):
...
async def update_seat_category(category_id: int, data: SeatCategoryUpdate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(is_superuser=True))):
    return await SeatCategoryCRUD.update(category_id, data, session)

@router.delete("/{category_id}")
async def delete_seat_category(category_id: int, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(is_superuser=True))):
