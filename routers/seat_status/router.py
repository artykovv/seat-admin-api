...
async def create_seat_status(data: SeatStatusCreate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
...
async def update_seat_status(status_id: int, data: SeatStatusUpdate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
    return await SeatStatusCRUD.update(status_id, data, session)

@router.delete("/{status_id}")
async def delete_seat_status(status_id: int, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
