...
async def create_seat(data: SeatCreate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(active=True))):
...
async def update_seat(seat_id: int, data: SeatUpdate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(active=True))):
    return await SeatCRUD.update(seat_id, data, session)

@router.delete("/{seat_id}")
async def delete_seat(seat_id: int, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(active=True))):
