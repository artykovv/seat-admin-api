...
async def create_floor(data: FloorCreate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
...
async def update_floor(floor_id: int, data: FloorUpdate, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
    return await FloorCRUD.update(floor_id, data, session)

@router.delete("/{floor_id}")
async def delete_floor(floor_id: int, session: AsyncSession = Depends(get_async_session), current_user: User = Depends(fastapi_users.current_user(superuser=True))):
